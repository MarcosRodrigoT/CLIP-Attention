...
def cross_similarity(text_embeddings):
    # Normalize the embeddings to have unit length
    normalized_text_embeddings = torch.nn.functional.normalize(text_embeddings, dim=1)  # [M, F]
    # Compute the cosine similarity matrix
    similarity_matrix = torch.mm(normalized_text_embeddings, normalized_text_embeddings.t())  # [M, M]

    return similarity_matrix


def plot_similarity(video, steps, similarity_matrix, method):
    ...
    ax.set_xticklabels(wrapped_steps, rotation=90, ha="center", va="top")
    ...
    # Save the plot
    plt.tight_layout()
    plt.savefig(f"results/steps_steps/{method}/{video.split('/')[-1].split('.')[0]}.png")
...
def main(METHOD):
    for i, video in enumerate(sorted(os.listdir("wikihow_val"))):
        ...
        similarity_matrix = cross_similarity(text_embeddings)

        # Convert the 2D matrix to an image
        plot_similarity(VIDEO, steps, similarity_matrix, METHOD)


if __name__ == "__main__":
    main(METHOD="from_filename")
    main(METHOD="from_transcript")
    main(METHOD="cvpr_paper_like")
